...
Конфликт #1: Схема изображений — ЗАКРЫТ

- **Проблема (ранее):** Риск рассинхрона при независимых миграциях в двух приложениях с одной БД
- **Решение:**
  - ✅ Single source of truth закреплён: миграции и seed выполняются только через `admin-panel`
  - ✅ В `ldm-steel` не запускаются prisma migrate deploy/seed в деплой-процессе (Dockerfile, пайплайны и инструкции это явно запрещают)
  - ✅ После миграций admin-panel проводится ручная smoke/prod-совместимость (см. deploy-runbook.md)

**Статус:** Политика миграций реализована корректно, риск рассинхрона устранён, конфликт закрыт.

### Конфликт #2: TypeScript strict mode — ЗАКРЫТ

- **Проблема (ранее):** Использование `as any` в prisma-client.ts для Neon adapter
- **Решение:**
  - ✅ Полностью устранено: в prisma/prisma-client.ts используется строгая типизация, обходов через any нет
  - ✅ Neon adapter интегрирован корректно, без потери типов

**Статус:** Проблема с any устранена, поддерживается zero-`as any` политика, конфликт закрыт.

### Конфликт #3: Environment variables — ЗАКРЫТ

- **Проблема (ранее):** Нет валидации переменных окружения (`DATABASE_URL`, `NEXTAUTH_SECRET`, `YOOKASSA_*`, `RESEND_API_KEY`)
- **Решение:**
  - ✅ В файле `shared/lib/env.ts` реализована строгая Zod-схема для всех обязательных переменных окружения
  - ✅ Валидация выполняется при старте приложения через импорт в `instrumentation.ts` (fail-fast)
  - ✅ Все критичные переменные объявлены как обязательные, опциональные — явно помечены
  - ✅ Ошибка валидации приводит к аварийному завершению с подробным описанием
  - ✅ Проверено: схема покрывает все production-критичные переменные, включая интеграции и базу данных

**Статус:** Валидация переменных окружения реализована корректно, конфликт закрыт.
